...
class AdminerShortcuts {
	...
	public function __construct() {
		$this->_isV5 = class_exists('Adminer\Adminer');
	}
	...
	public function selectQueryBuild($select, $where, $group, $order, $limit, $page) {
		...
		// https://github.com/vrana/adminer/blob/master/adminer/include/driver.inc.php#L80 (function select)
		if (!empty($_GET['select']) && empty($order) && !empty($this->_superOrder)) {
			...
			if ($this->_isV5)
				return 'SELECT' . Adminer\limit(
					($_GET['page'] != 'last' && $limit != '' && $group && $is_group && JUSH == 'sql' ? 'SQL_CALC_FOUND_ROWS ' : '') . implode(', ', $select) . "\nFROM " . Adminer\table($_GET['select']),
					($where ? "\nWHERE " . implode(' AND ', $where) : '') . ($group && $is_group ? "\nGROUP BY " . implode(', ', $group) : '') . "\nORDER BY " . implode(', ', $this->_superOrder),
					($limit != '' ? +$limit : null),
					($page ? $limit * $page : 0),
					"\n"
				);
	...
	public function tablesPrint($tables) {
		...
		// syntaxe Nowdoc
		$width = $this->_isV5 ? '100%' : 'calc(100% - 0.4em - 2px)';
		echo <<<HTML
<style>
#shortcuts { position:relative; margin:1.2em 1em 0; font-size:0.85em; line-height:1.2em; }
#shortcutsField { display:block; padding:0.2em; width:$width; border:1px solid #808080; }
...
#shortcutsEditField { display:block; padding:0.2em; width:calc(100% - 0.4em - 2px); border:1px solid #808080; }
